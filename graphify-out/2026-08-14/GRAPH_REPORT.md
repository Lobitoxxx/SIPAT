...
## Corpus Check
- 61 files · ~9,616,365 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 353 nodes · 498 edges · 34 communities (18 shown, 16 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 18 edges (avg confidence: 0.75)
...
4. `main()` - 11 edges
5. `Matriz Técnica SIPAT — Inventario de Datos (Fase 0)` - 11 edges
6. `3. Fichas detalladas` - 11 edges
7. `emergencias_coen()` - 10 edges
8. `_load()` - 10 edges
9. `_xy()` - 10 edges
10. `avisos_senamhi()` - 9 edges
...
Cohesion: 0.10
Nodes (41): alertas(), avisos(), health(), _nivel(), API FastAPI del modulo 'Ruta segura'. Endpoints: POST /ruta_segura {origin,…, ruta_segura(), ruta_segura_completa(), RutaRequest (+33 more)
...
Cohesion: 0.10
Nodes (18): _features(), _fit(), Tasa predictiva (siniestros/km) para cada punto muestreado de la ruta., Riesgo predictivo del modelo NegBin a lo largo de una ruta. Devuelve tasa media…, Replica la especificacion de features de Fase 1 (modelo_glm.py)., Entrena NegBin (Fase 1) y devuelve (modelo, pred por tramo)., RiesgoPredictivo, _haversine() (+10 more)
...
Cohesion: 0.09
Nodes (30): OSRMClient, Cliente HTTP para el router OSRM local (localhost:5000, perfil car). Uso desde…, coords: [(lon,lat), ...]; devuelve lista de dicts de rutas., get_predictor(), Capa predictiva: modelo NegBin de Fase 1 aplicado a rutas OSRM. -…, Composicion historica de los accidentes en el buffer de la ruta., tipos_incidente(), aadt_peajes() (+22 more)
...
## Knowledge Gaps
- **61 isolated node(s):** `graphify`, `Estado del proyecto (SIPAT)`, `1. Resumen ejecutivo`, `Pipeline de geocodificación (SUTRAN)`, `3.1 Magnitud y tendencia` (+56 more)
...
- **Why does `avisos_en_ruta()` connect `reporte_ruta.py` to `geocode.py`?**
